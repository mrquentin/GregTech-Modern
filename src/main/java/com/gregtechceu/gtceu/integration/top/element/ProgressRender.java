...
public class ProgressRender {
    ...
    public static void render(GuiGraphics guiGraphics, IProgressStyle style, int x, int y, int width, int height,
                              float progress, Component text) {
        RenderHelper.drawThickBeveledBox(guiGraphics, x, y, x + width, y + height, 1, style.getBorderColor(),
        ...
        if (progress > 0.0F) {
            ...
            if (style.getFilledColor() == style.getAlternatefilledColor()) {
                if (dx > 0) {
                    RenderHelper.drawThickBeveledBox(guiGraphics, x + 1, y + 1, x + dx + 1, y + height - 1, 1,
    ...
    public static void renderText(GuiGraphics graphics, int x, int y, int w, Component text, IProgressStyle style) {
        if (style.isShowText()) {
            ...
            switch (style.getAlignment()) {
                ...
                case ALIGN_CENTER -> RenderHelper.renderText(mc, graphics, (x + (w / 2)) - (textWidth / 2), y + 2,
                        text);
